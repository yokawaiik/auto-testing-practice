...
public class TestSuiteLimitedPokemonList extends JUnitTestBase {
  ...
  @Test
  @Tag("#TC002")
  @Description("Test Case #TC002: The number of Pokémon in the request is equal to the number of Pokémon in the response.")
  public void limitOfThePokemonShortList() {

    final var queryLimit = 100;

    final var limitedPokemonList = getApiClient().getLimitedPokemonList(queryLimit);

    Asserts.check(limitedPokemonList.limitedPokemonList.size() == queryLimit,
  ...
  public void maximumTheLimitOfThePokemonShortList() {
    ...
    final var maximumCount = getApiClient().getLimitedPokemonList(1).count; // 1279

    final var queryLimit = maximumCount;

    final var limitedPokemonList = getApiClient().getLimitedPokemonList(queryLimit);

    Asserts.check(limitedPokemonList.count == queryLimit && limitedPokemonList.limitedPokemonList.size() == queryLimit,
  ...
  @Test
  @Tag("#TC006")
  @Description("Test Case #TC006: Each pokemon has in limited list has a name (name).")
  public void eachPokemonInLimitedListHasName() {
    ...
    Asserts.check(
        limitedPokemonList.limitedPokemonList.stream().anyMatch(previewPokemon -> previewPokemon.name.length() != 0),
